...
parasphere.on('ready', () => {
  ...
  parasphere.user.setGame(">help | Shard 1/1", "phin-cdn.stream");
...
parasphere.on('message', msg => {
  ...
  if(msg.channel.type != "text") {
    var messagesn = {type: msg.channel.type, messageID: msg.id, userID: msg.author.id, channelID: msg.channel.id, message: msg.content, timestamp: new Date().getTime()};
  } else {
    var messagesn = {type: msg.channel.type, messageID: msg.id, userID: msg.author.id, channelID: msg.channel.id, serverID: msg.guild.id, message: msg.content, timestamp: new Date().getTime()};
  }
  ...
  dbApi.get({guildID: gc.id}, 'guilds', function(err, res) {
    if(!err) {
      if(!res) {
        ...
        dbApi.insert(dbApi.dbC, 'guilds', guildsn);
