...
parasphere.on('ready', () => {
  ...
  dbApi.dbConnect(function(db) {
    var launchsn = {
      instanceIP: internalIP,
      currStatus: parasphere.status,
      guilds: parasphere.guilds.size,
      readyAt: parasphere.readyAt
    };
...
parasphere.on('guildDelete', gc => {
  ...
  dbApi.update(dbApi.dbC, 'guilds', idObj, {
    $set: {
      inCord: false
    }
  });
